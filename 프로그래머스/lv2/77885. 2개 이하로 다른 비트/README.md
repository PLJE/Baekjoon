...
### 풀이 
예시만 보고 하다가는 함정에 빠진다..ㅏ.. 
최대 2개를 바꿀 수 있다.   
비트를 뒤에서부터 보면서 0이 있다면 그 자리를 1로 바꿔준다. (여기서 끝내면 안된다)   
방금 바꾼 자리 + 1 이 만약 1 이라면 0으로 바꿔줘야 한다. 자기 보다 큰 수 중에서 가장 작은 수를 구해야 하기 때문에    
뒤에서부터 탐색 했지만 0이 하나도 없다면, 제일 앞자리 1을 0으로 바꾸고 앞에 1을 추가한 수가 큰 수 중 가장 작은 수가 된다.   
