...
### 풀이
제한을 보면 O(N)으로 푸는 풀이를 떠올려야 할 것이라는 생각이 든다.   
작은 수 풍선은 자신보다 큰 수의 풍선을 다 제거할 수 있다는 것을 떠올려야 한다.   
어떤 풍선을 남기려고 할 때, 그 양쪽을 본다.   
왼쪽, 오른쪽의 최소값이 현재 풍선 값보다 더 작다면, 한 번의 횟수(더 작은걸 터뜨리는 것)로 그 풍선만 남길 수가 없다   
누적합과 유사한 개념으로 양쪽에서 시작해서 최소값을 구해서 푸는 문제였다. 
