...
### 내 풀이
오랜만에 트리 문제라서 구조체 선언부터 트리 구현까지 시간이 걸렸다.  
root부터 보면서 x값을 비교하여 insert하는 함수에서 while 문을 돌며 left, right 트리로 내려가 하나씩 노드를 추가한다.   
이 문제의 가장 큰 특징은, 각 노드의 y좌표 즉 depth가 이미 정해져 있다.   
그렇기 때문에, 노드를 하나씩 삽입하기 전에, y좌표에 대해 내림차순으로 정렬 후, y가 큰 것부터(depth가 얕은 것 부터) 삽입하여 x값을 비교하며 left, right 에 노드를 추가한다.   
트리 구현 후에는 전위순회(preorder), 후위순회(postorder) 한 것을 구하면 된다. 순회의 경우는 재귀로 구현한다.    
