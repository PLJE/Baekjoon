...
### 내 풀이
행과 열에 대해 각각 경우를 따져서 구하면 되는 구현 문제이다.    
문제 설명이 애매한 감이 있다. 처음에 가로에 대해 경사로를 놓았을 때, 세로 케이스에서 가로에서 이미 경사로를 설치한 길이면 설치하지 못하도록 했는데, 여기서는 행과 열에 대해 모든 케이스를 구하는 경우라서 따로 봐야 한다.    
예제를 보면 알 수 있긴 하지만, 지문만 보면 헷갈릴 수 있다   
visit 배열을 행과 열에 대해 다르게 적용해주면 된다.   
