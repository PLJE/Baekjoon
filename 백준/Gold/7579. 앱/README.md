...
### 풀이
오랜만에 냅색 문제임   
해당 문제의 경우 하나의 아이템은 한번만 쓸 수있다 (0-1 knapsack). 근데 처음에 unbounded로 접근했다.    
2차원 배열에서 한 축은 아이템이고, 나머지 한축을 설정하려고 보면, 용량으로 하면 최대가 10,000,000*100 이라 안되기 때문에 cost로 설정하면 된다   
cost의 최대는 100*100 임 
