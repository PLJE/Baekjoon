...
### 풀이
1. R, B가 평행할 경우 처리   
이 때, R B 배치를 모두 고려해서 그 경우마다 어떻게 되는지 처리하려고 해서 막혔다   
그냥 벽을 만날 때 까지 둘다 굴리고, 이 때 두 구슬의 좌표가 완전히 같아지는데, 도달하기까지 걸린 거리가 더 긴 것이 한 칸 뒤에 있는 것으로 처리해주면 쉽다   
2. visit 배열 설정 
이것 때문에 중간에서 실패했다   
visit[11][11][2] 로 두고, visit[i][j][k] (k : R or B) 좌표 (i,j) 에 해당 색 공이 위치할 경우 visit으로 처리했다   
어떤 공이 특정 위치에 두번 오더라도, 나머지 다른 구슬이 두 번 다른 위치라면 이는 다른 것으로 처리해줘야 하는데 위의 경우 처리가 불가능하다   
visit[i][j][k][l] 로 R 이 (i,j) B가 (k,l) 좌표일 때 board 상태의 visit을 처리해준다
