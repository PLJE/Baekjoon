...
### 내 풀이
- 주사위의 각 면에 idx를 할당하여 각 면에 어떤 숫자가 있는지 `dice[7]`배열에 저장   
- 각 면`(1~6)` 별로 `네 방향(1~4)`으로 굴렸을 때, 각 면에 있던 숫자가 몇 번으로 가는지 2차원 배열에 저장   
![image](https://user-images.githubusercontent.com/77712822/226167098-36300eac-264c-4dee-a7ae-1c9716139843.png)
- ex: `mv[6][2] == 3` 2번(서쪽)으로 굴렸을 때 6번 면(내 경우 바닥면)의 숫자는 3번 면(오른쪽)으로 가게 됨
- 굴릴 때 마다 배열을 참조하여 모든 면의 숫자를 알맞은 면으로 이동시키고, 문제 조건 그대로 구현해주면 된다.    
