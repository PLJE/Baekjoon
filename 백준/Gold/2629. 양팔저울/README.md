...
### 풀이
문제(앱) 풀고 냅색 분류에서 비슷한 난이도 문제 하나 더 찾아서 풀었다   
양팔저울에 추를 올려서 잴 수 있는 구슬의 무게를 판단하는 것    
3차원 배열을 선언해서 dp[i][j][k] i부터 j까지 추 중에서 사용하여 k 구슬 무게 판단이 가능하면 1을 저장한다.   
dp[1][1][첫번째 추 무게] 부터 시작하여 재귀함수를 돈다.   
이미 1 이 있다면 재귀를 더 돌지 않게 return해준다.    
