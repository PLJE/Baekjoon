...
### 풀이
dfs, 시뮬레이션 문제이며, 호흡이 긴 문제여서 세부적인 부분에서 구현 실수가 나지 않게 해야함   
핵심은 2차원 배열을 rotate하는 것   
시간 제한이 교묘함. 시간초과가 뜨다가, L=0 일 때, 즉 길이 1 에 대해 rotate하는 것(그대로 두는 것)일 때는 rotate함수를 돌리지 않게 고쳤더니 통과했다.   
이때 rotate를 돌리면 모든 칸에 대해 돌기 때문 
